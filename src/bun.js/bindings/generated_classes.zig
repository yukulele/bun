...
pub const JSPostgresSQLQuery = struct {
    ...
    extern fn PostgresSQLQueryPrototype__instanceSetCachedValue(JSC.JSValue, *JSC.JSGlobalObject, JSC.JSValue) void;

    extern fn PostgresSQLQueryPrototype__instanceGetCachedValue(JSC.JSValue) JSC.JSValue;

    /// `PostgresSQLQuery.instance` setter
    /// This value will be visited by the garbage collector.
    pub fn instanceSetCached(thisValue: JSC.JSValue, globalObject: *JSC.JSGlobalObject, value: JSC.JSValue) void {
        JSC.markBinding(@src());
        PostgresSQLQueryPrototype__instanceSetCachedValue(thisValue, globalObject, value);
    }

    /// `PostgresSQLQuery.instance` getter
    /// This value will be visited by the garbage collector.
    pub fn instanceGetCached(thisValue: JSC.JSValue) ?JSC.JSValue {
        JSC.markBinding(@src());
        const result = PostgresSQLQueryPrototype__instanceGetCachedValue(thisValue);
        if (result == .zero)
            return null;
